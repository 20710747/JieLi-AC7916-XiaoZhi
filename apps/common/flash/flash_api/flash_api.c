...
#define USE_CRC32	1 //使用CRC校验，不使用则默认和校验
    ...
    u32 start_addr = user_get_flash_exif_addr();//起始地址
    if (start_addr == 0)
    {
        return;
    }
    u32 end_addr = start_addr + 256 * 1024;//结束地址
...
void flash_read_write_init(void)
{
    ...
    flash_start_addr = user_get_flash_exif_addr();
    flash_end_addr = flash_start_addr + 64 * 4096;

    DBG_PRINT("---> flash id = ", flash_id);
    DBG_PRINT("---> flash cap_size(KB) = ", cap_size / 1024);
    DBG_PRINT("---> flash flash_start_addr = ", flash_start_addr);
    DBG_PRINT("---> flash flash_end_addr = ", flash_end_addr);
}
...
#define FLASH_BLOCK 4096
    ...
    //DBG_PRINT("flash_write_buff addr = ",addr);
    //DBG_PRINT("flash_write_buff len = ",len);
    //DBG_PRINT("flash_write_buff beginBlock = ",beginBlock);
    //DBG_PRINT("flash_write_buff endBlock = ",endBlock);
...
int flash_read_buff(u8 *buff, u32 addr, u32 len)
{
    ...
    if(addr + len > flash_end_addr)
        len = flash_end_addr - addr;
    //DBG_PRINT("norflash_origin_read addr = ",addr);
    //DBG_PRINT("norflash_origin_read len = ",len);
    ret = norflash_origin_read(buff, addr, len);
    //DBG_PRINT("norflash_origin_read ret = ",ret);
    return 0;
}
//=======================================用户flash API读写函，用户可直接使用===================================================//
