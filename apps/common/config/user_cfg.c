...
static char ble_name2[LOCAL_NAME_LEN];
...
char *bt_get_local_name(void)
{
    ...
    sprintf(edr_name, "michiko-%02X%02X", mac_addr[4], mac_addr[5]);
#endif
    return edr_name;
}
...
char *ble_get_local_name(void)
{
    ...
    sprintf(ble_name2, "bleQQab_%02X%02X", mac_addr[4], mac_addr[5]);
    for(i=8;ble_name2[i];i++)
    {
        if((ble_name2[i]>='A')&&(ble_name2[i]<='F'))
            ble_name2[i] = '0' + (ble_name2[i]-'A');
        else if((ble_name2[i]>='a')&&(ble_name2[i]<='b'))
            ble_name2[i] = '0' + (ble_name2[i]-'a');
        else if((ble_name2[i]>='0')&&(ble_name2[i]<='9'))
            ble_name2[i] = 'a' + (ble_name2[i]-'0');
        else
            ble_name2[i] = 'Q';
    }
#endif
