...
void bt_net_config_ssid_pwd_devicecode_notify()
{
    struct net_event net;
    net.arg = "net";
    net.event = NET_EVENT_SMP_CFG_FINISH;
...
void bt_net_config_set_ssid_pwdEX(const char *pp_ssid, const char * pp_password)
{
    memset(&bt_net_result, 0, sizeof(bt_net_result));
    strcpy(bt_net_result.ssid, pp_ssid);
    strcpy(bt_net_result.pwd, pp_password);
}
