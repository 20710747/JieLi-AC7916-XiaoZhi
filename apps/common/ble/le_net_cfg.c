...
#define ATT_LOCAL_PAYLOAD_SIZE    (500)                   //note: need >= 20
...
extern void bt_net_config_ssid_pwd_devicecode_notify();
extern void bt_net_config_set_ssid_pwdEX(const char *pp_ssid, const char * pp_password);
...
enum
{
    IFLY_AIUI_BLE_CFG_MSG_TYPE_HANDSHAKE=0,
    IFLY_AIUI_BLE_CFG_MSG_TYPE_NETCFG,
    IFLY_AIUI_BLE_CFG_MSG_TYPE_BIND,
    IFLY_AIUI_BLE_CFG_MSG_TYPE_END,
    IFLY_AIUI_BLE_CFG_MSG_TYPE_QUERY,
    IFLY_AIUI_BLE_CFG_MSG_TYPE_UNKNOWN,
};

//status 值，0：成功，1：失败，2：正在配网，3：未找到 WIFI 名称，4：密码错误
enum
{
    IFLY_AIUI_BLE_CFG_STATUS_SUCCESS=0,
    IFLY_AIUI_BLE_CFG_STATUS_FAIL,//1
    IFLY_AIUI_BLE_CFG_STATUS_RECEIVED_SSID,//2
    IFLY_AIUI_BLE_CFG_STATUS_NOTFINDSSID,//3
    IFLY_AIUI_BLE_CFG_STATUS_PWDERROR,//4
    IFLY_AIUI_BLE_CFG_STATUS_UNKNOWN,
};
...
enum
{
    IFLY_AIUI_BIND_STATE_SUCCESS = 0,//绑定成功
    IFLY_AIUI_BIND_STATE_BINDED,//已绑定
    IFLY_AIUI_BIND_STATE_FAIL,//绑定失败
    IFLY_AIUI_BIND_STATE_BINDING,//待绑定
    IFLY_AIUI_BIND_STATE_UNKNOWN,
    IFLY_AIUI_BIND_STATE_BLE_BINDED,//小程序告诉已绑定
};

typedef struct
{
    unsigned char v_wifi_state;
    unsigned char v_bind_state;
    char a_device_code[IFLY_AIUI_DEVICE_CODE_LEN + 1];
} t_ifly_aiui_net_config_info;

static t_ifly_aiui_net_config_info gs_ifly_aiui_net_cfg_info =
{
    .v_wifi_state = IFLY_AIUI_BLE_CFG_STATUS_UNKNOWN,
    .v_bind_state = IFLY_AIUI_BIND_STATE_UNKNOWN,
    .a_device_code[0] = 0,
};
...
static uint16_t att_read_callback(hci_con_handle_t connection_handle, uint16_t att_handle, uint16_t offset, uint8_t *buffer, uint16_t buffer_size)
{
    ...
    printf("read_callback, handle= 0x%04x,buffer= 0x%x, buffer_size=%d\n", handle, (u32)buffer, buffer_size);
...
static void ifly_aiui_net_cfg_send_connecting_resp(unsigned char vp_msg_type, unsigned char vp_status)
{
    unsigned char *p;
    unsigned char len;
    ...
    *p++ = 0x10;
    *p++ = 0x02;
    if(vp_msg_type == IFLY_AIUI_BLE_CFG_MSG_TYPE_HANDSHAKE)
    {
        len = sprintf((char *)p, "{\"key\":\"%s\"}", "XFAIBLE_ACK");
    }
    else if(vp_msg_type == IFLY_AIUI_BLE_CFG_MSG_TYPE_NETCFG)
    {
        if(vp_status == IFLY_AIUI_BLE_CFG_STATUS_RECEIVED_SSID)
        {
            char *pl_device_id, *pl_sn;
            unsigned char mac_addr[7];
            char mac_addr_buf[20];

            pl_device_id = ifly_aiui_sdk_common_get_deviceid();
            pl_sn = ifly_aiui_sdk_common_get_deviceid();
            wifi_get_mac(mac_addr);
            sprintf(mac_addr_buf, "%02X-%02X-%02X-%02X-%02X-%02X", mac_addr[0], mac_addr[1], mac_addr[2], mac_addr[3], mac_addr[4], mac_addr[5]);
            len = sprintf((char *)p, "{\"key\":\"%s\",\"status\":%d,", "SSID_ACK", vp_status);

            len += sprintf(p+len, "\"data\":{\"deviceId\":\"%s\",\"sn\":\"%s\",\"mac\":\"%s\"}}", pl_device_id, pl_sn, mac_addr_buf);
        }
        else
        {
            len = sprintf((char *)p, "{\"key\":\"%s\",\"status\":%d}", "SSID_ACK", vp_status);
    ...
    else if(vp_msg_type == IFLY_AIUI_BLE_CFG_MSG_TYPE_BIND)
    {
        len = sprintf((char *)p, "{\"key\":\"%s\",\"status\":%d}", "BIND_ACK", vp_status);
    }
    else if(vp_msg_type == IFLY_AIUI_BLE_CFG_MSG_TYPE_END)
    {
        len = sprintf((char *)p, "{\"key\":\"%s\"}", "BLEEND_ACK");
    }
    else if(vp_msg_type == IFLY_AIUI_BLE_CFG_MSG_TYPE_QUERY)
    {
        len = sprintf((char *)p, "{\"key\":\"%s\",\"WIFI\":%d,\"BIND\":%d}", "QUERY_ACK", gs_ifly_aiui_net_cfg_info.v_wifi_state, gs_ifly_aiui_net_cfg_info.v_bind_state);
    }
    else
    {
        return;
    ...
    p += len;
    len += 2;//Length = Type + Value， 不包括固定的Crc16和End。长度最大是1024
    buf[3] = len;//此时写长度
    u16 vl_crc16 = CRC16(&buf[4], len);
    *p++ = (unsigned char)(vl_crc16 >>8);
    *p++ = (unsigned char)(vl_crc16 & 0xff);
    *p = 0xFF;
    len += 4+3;
    printf("ifly_aiui_net_cfg_send_connecting_resp, len=%d, json_str=%s, packet_buf=\r\n", len, (char *)&buf[6]);
    put_buf(buf, len);
    app_send_user_data(ATT_CHARACTERISTIC_ae82_01_VALUE_HANDLE, buf, len, ATT_OP_AUTO_READ_CCC);
}

int ifly_aiui_net_cfg_message_parse(const char *data)
{
    json_object *pl_jsonRoot, *pl_json, *pl_json_key, *pl_json_data;
    char *pl_key, *pl_str, *pl_str2;

    pl_jsonRoot = json_tokener_parse(data);
    if (pl_jsonRoot == NULL)
    {
        return -1;
    }

    pl_json_key = json_object_object_get(pl_jsonRoot, "key");
    if(pl_json_key)
    {
        pl_key = json_object_get_string(pl_json_key);

        if(strcmp(pl_key, "XFAIBLE") == 0)
        {
            gs_ifly_aiui_net_cfg_info.v_wifi_state = IFLY_AIUI_BLE_CFG_STATUS_UNKNOWN;
            gs_ifly_aiui_net_cfg_info.v_bind_state = IFLY_AIUI_BIND_STATE_UNKNOWN;
            ifly_aiui_net_cfg_send_connecting_resp(IFLY_AIUI_BLE_CFG_MSG_TYPE_HANDSHAKE, 0);
        }
        else if(strcmp(pl_key, "BLEEND") == 0)
        {
            ifly_aiui_net_cfg_send_connecting_resp(IFLY_AIUI_BLE_CFG_MSG_TYPE_END, 0);
            sys_timeout_add(0, (void (*)(void *))ble_module_enable, 100);
        }
        else if(strcmp(pl_key, "BIND") == 0)
        {
            gs_ifly_aiui_net_cfg_info.v_bind_state = IFLY_AIUI_BIND_STATE_UNKNOWN;
            pl_json_data = json_object_object_get(pl_jsonRoot, "data");
            if(pl_json_data)
            {
                #if defined(CONFIG_APP_HANDE_FTR) || defined(CONFIG_APP_HANDE_ICAL_FTR)
                int vl_homeID_status = 0;
                pl_json = json_object_object_get(pl_json_data, "homeId");
                if(pl_json)
                {
                    pl_str = json_object_get_string(pl_json);
                    int ret = user_app_write_homeID(pl_str);
                    if(ret == 0)
                    {
                        vl_homeID_status = 1;
                        printf("ifly_aiui_net_cfg_message_parse, user_app_write_homeID, homeid=%s, ret=%d\r\n", pl_str, ret);
                    }
                }

                if(vl_homeID_status != 0)
                {
                    gs_ifly_aiui_net_cfg_info.v_bind_state = IFLY_AIUI_BIND_STATE_BINDED;
                    complete = 1;
                    bt_net_config_ssid_pwd_devicecode_notify();
                }
                #else
                pl_json = json_object_object_get(pl_json_data, "status");
                if(pl_json)
                {
                    int vl_status = json_object_get_int(pl_json);
                    if(vl_status ==0)
                    {
                        pl_json = json_object_object_get(pl_json_data, "deviceCode");
                        if(pl_json)
                        {
                            pl_str = json_object_get_string(pl_json);
                            printf("ifly_aiui_net_cfg_message_parse,str_device_code=%s\r\n", pl_str);

                            int len = strlen(pl_str);
                            if(len <= IFLY_AIUI_DEVICE_CODE_LEN)
                            {
                                memcpy(gs_ifly_aiui_net_cfg_info.a_device_code, pl_str, len);
                                gs_ifly_aiui_net_cfg_info.a_device_code[len] = 0;
                                gs_ifly_aiui_net_cfg_info.v_bind_state = IFLY_AIUI_BIND_STATE_BINDING;
                            }
                        }
                    }
                    else
                    {
                        gs_ifly_aiui_net_cfg_info.v_bind_state = IFLY_AIUI_BIND_STATE_BLE_BINDED;
                    }
                }
                #endif
            }
        }
        else if(strcmp(pl_key, "SSID") == 0)
        {
            pl_json_data = json_object_object_get(pl_jsonRoot, "data");
            if(pl_json_data)
            {
                pl_json = json_object_object_get(pl_json_data, "ssid");
                if(pl_json)
                {
                    pl_str = json_object_get_string(pl_json);
                    pl_json = json_object_object_get(pl_json_data, "pass");
                    if(pl_json)
                    {
                        pl_str2 = json_object_get_string(pl_json);
                        bt_net_config_set_ssid_pwdEX(pl_str, pl_str2);
                        printf("ifly_aiui_net_cfg_message_parse,ssid=%s, pwd=%s\r\n", pl_str, pl_str2);
                        gs_ifly_aiui_net_cfg_info.v_wifi_state = IFLY_AIUI_BLE_CFG_STATUS_RECEIVED_SSID;
                        ifly_aiui_net_cfg_send_connecting_resp(IFLY_AIUI_BLE_CFG_MSG_TYPE_NETCFG, IFLY_AIUI_BLE_CFG_STATUS_RECEIVED_SSID);
                        complete = 1;
        ...
        else if(strcmp(pl_key, "QUERY") == 0)
        {
            ifly_aiui_net_cfg_send_connecting_resp(IFLY_AIUI_BLE_CFG_MSG_TYPE_QUERY, 0);
        }
    }
    json_object_put(pl_jsonRoot);
    return -1;
}

static void check_net_info_if_recv_complete()
{
    if (user_buf[user_buf_offset - 1] == 0xFF && user_buf_offset == user_data_size + 7)
    {
        ...
        if (crc16 == CRC16(&user_buf[2], user_data_size + 2))
        {
            ...
            printf("check_net_info_if_recv_complete, json_str=%s\r\n", &user_buf[6]);

            ifly_aiui_net_cfg_message_parse(&user_buf[6]);
        ...
        else
        {
            ...
            gs_ifly_aiui_net_cfg_info.v_wifi_state = IFLY_AIUI_BLE_CFG_STATUS_FAIL;
            ifly_aiui_net_cfg_send_connecting_resp(IFLY_AIUI_BLE_CFG_MSG_TYPE_NETCFG, IFLY_AIUI_BLE_CFG_STATUS_FAIL);//配网失败
            user_buf_offset = 0;
            user_data_size = 0;
            memset(user_buf, 0, sizeof(user_buf));
        }
...
static int att_write_callback(hci_con_handle_t connection_handle, uint16_t att_handle, uint16_t transaction_mode, uint16_t offset, uint8_t *buffer, uint16_t buffer_size)
{
    ...
    case ATT_CHARACTERISTIC_ae82_01_CLIENT_CONFIGURATION_HANDLE:
        ...
        complete = 0;
        break;
    //case ATT_CHARACTERISTIC_ae84_01_VALUE_HANDLE:
    case ATT_CHARACTERISTIC_ae81_01_VALUE_HANDLE:
        printf("\nhandle=0x%x, rx_size(%d):", handle, buffer_size);
...
static int app_send_user_data(u16 handle, const u8 *data, u16 len, u8 handle_type)
{
    ...
    printf("app_send_user_data, ret=%d\r\n", ret);
    return ret;
}
...
static int make_set_adv_data(void)
{
    ...
    char *pl_info = ifly_aiui_sdk_common_get_appid();
    temp_buf[0] = 0xAA;
    temp_buf[1] = 0xAA;
    //int len = sprintf(&temp_buf[2], "%s%s", "XFAI-BLE-",pl_info);
    int len = sprintf(&temp_buf[2], "%s%s", "XFAI-BLE-","f3821705");
    offset += make_eir_packet_data(&buf[offset], offset, HCI_EIR_DATATYPE_MANUFACTURER_SPECIFIC_DATA, temp_buf, len + 2);
...
{
    printf("ble_cfg_net_result_notify, complete=%d, event=%d\r\n", complete, event);
    if (!complete)
    {
        return;
    }
    ...
    if (event == NET_CONNECT_TIMEOUT_NOT_FOUND_SSID)
    {
        ...
        gs_ifly_aiui_net_cfg_info.v_wifi_state = IFLY_AIUI_BLE_CFG_STATUS_NOTFINDSSID;
        ifly_aiui_net_cfg_send_connecting_resp(IFLY_AIUI_BLE_CFG_MSG_TYPE_NETCFG, IFLY_AIUI_BLE_CFG_STATUS_NOTFINDSSID);//未找到 WIFI 名称
    }
    else if (event == NET_CONNECT_ASSOCIAT_TIMEOUT || event == NET_CONNECT_ASSOCIAT_FAIL)
    {
        //app_send_user_data(ATT_CHARACTERISTIC_ae82_01_VALUE_HANDLE, rsp_cmd4, sizeof(rsp_cmd4), ATT_OP_AUTO_READ_CCC);
        gs_ifly_aiui_net_cfg_info.v_wifi_state = IFLY_AIUI_BLE_CFG_STATUS_PWDERROR;
        ifly_aiui_net_cfg_send_connecting_resp(IFLY_AIUI_BLE_CFG_MSG_TYPE_NETCFG, IFLY_AIUI_BLE_CFG_STATUS_PWDERROR);//密码错误
    }
    else if (event == NET_EVENT_CONNECTED)
    {
        printf("ble_cfg_net_result_notify, NET_EVENT_CONNECTED, send rsp_cmd0\r\n");
        //app_send_user_data(ATT_CHARACTERISTIC_ae82_01_VALUE_HANDLE, rsp_cmd0, sizeof(rsp_cmd0), ATT_OP_AUTO_READ_CCC);
        ifly_aiui_net_cfg_send_connecting_resp(IFLY_AIUI_BLE_CFG_MSG_TYPE_NETCFG, IFLY_AIUI_BLE_CFG_STATUS_SUCCESS);//配网成功
        gs_ifly_aiui_net_cfg_info.v_wifi_state = IFLY_AIUI_BLE_CFG_STATUS_SUCCESS;
        complete = 0;
#if defined(CONFIG_APP_HANDE_FTR) || defined(CONFIG_APP_HANDE_ICAL_FTR)
        if(gs_ifly_aiui_net_cfg_info.v_bind_state == IFLY_AIUI_BIND_STATE_BINDED)
#else
        if(gs_ifly_aiui_net_cfg_info.v_bind_state == IFLY_AIUI_BIND_STATE_BINDING || gs_ifly_aiui_net_cfg_info.v_bind_state == IFLY_AIUI_BIND_STATE_BLE_BINDED)
#endif
        {
            int ret;
            char *pl_sn;
            ...
            wifi_get_mac(mac_addr);
            sprintf(mac_addr_buf, "%02X-%02X-%02X-%02X-%02X-%02X", mac_addr[0], mac_addr[1], mac_addr[2], mac_addr[3], mac_addr[4], mac_addr[5]);
            ret = 0;//ifly_aiui_device_active(mac_addr_buf, pl_sn, "", "");
            printf("ifly_aiui_device_active, ret=%d\r\n", ret);
            if(ret != 0)
            {
                ifly_aiui_net_cfg_send_connecting_resp(IFLY_AIUI_BLE_CFG_MSG_TYPE_BIND, IFLY_AIUI_BIND_STATE_FAIL);
                gs_ifly_aiui_net_cfg_info.v_bind_state = IFLY_AIUI_BIND_STATE_FAIL;
            }
            else
            {
                #if defined(CONFIG_APP_HANDE_FTR) || defined(CONFIG_APP_HANDE_ICAL_FTR)
                ifly_aiui_net_cfg_send_connecting_resp(IFLY_AIUI_BLE_CFG_MSG_TYPE_BIND, IFLY_AIUI_BIND_STATE_BINDED);
                #else
                if(gs_ifly_aiui_net_cfg_info.v_bind_state == IFLY_AIUI_BIND_STATE_BINDING)
                {
                    ret = 0;//ifly_aiui_device_bind(gs_ifly_aiui_net_cfg_info.a_device_code);
                    printf("ifly_aiui_device_bind, ret=%d\r\n", ret);
                    if(ret == 0)
                    {
                        ifly_aiui_net_cfg_send_connecting_resp(IFLY_AIUI_BLE_CFG_MSG_TYPE_BIND, IFLY_AIUI_BIND_STATE_SUCCESS);
                        gs_ifly_aiui_net_cfg_info.v_bind_state = IFLY_AIUI_BIND_STATE_SUCCESS;
                    }
                    else
                    {
                        ifly_aiui_net_cfg_send_connecting_resp(IFLY_AIUI_BLE_CFG_MSG_TYPE_BIND, IFLY_AIUI_BIND_STATE_FAIL);
                ...
                else if(gs_ifly_aiui_net_cfg_info.v_bind_state == IFLY_AIUI_BIND_STATE_BLE_BINDED)
                {
                    ifly_aiui_net_cfg_send_connecting_resp(IFLY_AIUI_BLE_CFG_MSG_TYPE_BIND, IFLY_AIUI_BIND_STATE_BINDED);
                    gs_ifly_aiui_net_cfg_info.v_bind_state = IFLY_AIUI_BIND_STATE_BINDED;
                }
...
#if 0//!defined PAYMENT_AUDIO_SDK_ENABLE && !CONFIG_POWER_ON_ENABLE_BLE
        sys_timeout_add(0, (void (*)(void *))ble_module_enable, 100);
#endif
    }
}
